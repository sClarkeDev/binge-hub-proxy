...
function copyHeader(headers: Headers, outputHeaders: Headers, inputKey: string, outputKey: string) {
  if (headers.has(inputKey)) outputHeaders.set(outputKey, headers.get(inputKey) ?? '');
}
...
export function getProxyHeaders(headers: Headers): Headers {
  ...
  output.set('User-Agent', 'Mozilla/5.0 (Windows NT 10.0; Win64; x64; rv:93.0) Gecko/20100101 Firefox/93.0');

  return output;
}

export function getAfterResponseHeaders(headers: Headers, finalUrl: string): Record<string, string> {
  const output: Record<string, string> = {};

  if (headers.has('Set-Cookie')) output['X-Set-Cookie'] = headers.get('Set-Cookie') ?? '';
...
export function removeHeadersFromEvent(event: H3Event, key: string) {
  ...
  if (event.node.req.headers[normalizedKey]) delete event.node.req.headers[normalizedKey];
}
